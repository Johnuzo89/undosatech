...
// FIXED: setStudies handles both array and {studies:[]} responses
//        log timestamp falls back to logged_at field

import { useState, useEffect, useRef, useCallback } from "react";

const ORCHESTRATOR = import.meta.env.VITE_API_URL || "https://undosatech-production.up.railway.app";
...
const STATUS_CONFIG = {
  ...
  cancelled: { color: "#f59e0b", bg: "rgba(245,158,11,0.1)",  icon: "■",  label: "Cancelled" },
  cancelling:{ color: "#f59e0b", bg: "rgba(245,158,11,0.1)",  icon: "■",  label: "Cancelling" },
  deleted:   { color: "#6b7280", bg: "rgba(107,114,128,0.1)", icon: "✗",  label: "Deleted" },
};
...
function ProgressBar({ pct, status }) {
  const color = status === "completed" ? "#22d3a5"
              ...
              : (status === "stopped" || status === "cancelled") ? "#f59e0b"
              : "#38bdf8";
  return (
        ...
        logs.map(l => (
          <div key={l.id} style={{ color: levelColor[l.level] || "#94a3b8" }}>
            <span style={{ color: "#475569", userSelect: "none" }}>
              [{new Date(l.timestamp ?? l.logged_at).toLocaleTimeString()}]
            </span>{" "}
            {l.message}
          </div>
        ))
...
function StudyCard({ study, session, onStop, onDelete }) {
  ...
  useEffect(() => {
    ...
    const fetchFull = async () => {
      try {
        const res = await fetch(`${ORCHESTRATOR}/studies/${id}/status`, {
          headers: { Authorization: `Bearer ${session.access_token}` },
        });
  ...
  }, [expanded, isRunning, id, session]);

  const handleStop = async (e) => {
    e.stopPropagation(); setStopping(true);
    await onStop(id); setStopping(false);
  };
  ...
  // per_class_accuracy may be dict or array — normalise to dict
  const perClassDict = (() => {
    const pca = fullStudy?.per_class_accuracy;
    if (!pca) return null;
    if (Array.isArray(pca)) {
      return Object.fromEntries(pca.map((v, i) => [`Class ${i}`, v]));
    }
    return pca;
  })();
  ...
  return (
    <div style={{
      ...
      <div onClick={() => setExpanded(x => !x)} style={{ padding: "16px 18px", cursor: "pointer" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 10 }}>
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 4, flexWrap: "wrap" }}>
              ...
              {study.dp_enabled && (
                <span style={{ fontSize: 10, padding: "2px 7px", borderRadius: 99,
                  background: "rgba(167,139,250,0.12)", color: "#a78bfa", fontWeight: 600 }}>🔒 DP</span>
              )}
            </div>
            ...
            <p style={{ margin: 0, fontSize: 12, color: "#64748b" }}>
              {study.model || study.architecture} · {study.dataset}
        ...
        <div style={{ display: "flex", justifyContent: "space-between", fontSize: 11, color: "#475569" }}>
          ...
          <span>{study.final_accuracy != null
        ...
        <div style={{ borderTop: "1px solid rgba(255,255,255,0.06)", padding: "14px 18px" }}>
          <div style={{ marginBottom: 14 }}>
            <div style={{ fontSize: 11, fontWeight: 600, color: "#475569", marginBottom: 6,
              textTransform: "uppercase", letterSpacing: "0.05em" }}>Participating Nodes</div>
            <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
              {study.nodes?.map(n => (
                <span key={n} style={{ fontSize: 11, padding: "3px 10px", borderRadius: 99,
                  background: "rgba(56,189,248,0.08)", color: "#7dd3fc",
                  border: "1px solid rgba(56,189,248,0.15)" }}>{n}</span>
              ))}
            </div>
          </div>
          ...
          {fullStudy && (
            <div style={{ display: "grid", gridTemplateColumns: "auto 1fr", gap: 16, marginBottom: 14, alignItems: "start" }}>
              ...
              {perClassDict && Object.keys(perClassDict).length > 0 && (
                <div>
                  <div style={{ fontSize: 11, color: "#475569", marginBottom: 6, textTransform: "uppercase", letterSpacing: "0.05em" }}>Per-class</div>
                  <div style={{ display: "flex", flexWrap: "wrap", gap: 4 }}>
                    {Object.entries(perClassDict).map(([cls, acc]) => (
                      <span key={cls} style={{ fontSize: 10, padding: "2px 7px", borderRadius: 99,
          ...
          <div style={{ marginBottom: fullStudy?.status === "completed" ? 14 : 0 }}>
            ...
            <LiveLogs studyId={id} session={session} isRunning={isRunning} />
          </div>

          {fullStudy?.status === "completed" && fullStudy?.model_download_path && (
            <a href={`${ORCHESTRATOR}/studies/${id}/download`} style={{
...
export default function MyStudies({ session }) {
  ...
  const fetchStudies = useCallback(async () => {
    ...
    try {
      ...
      const data = await res.json();
      // FIX: API may return array or {studies:[]}
  ...
  const handleDelete = async (studyId) => {
    ...
    setStudies(prev => prev.filter(s => (s.id || s.study_id) !== studyId));
  };

  const filtered = filter === "all" ? studies : studies.filter(s => s.status === filter);
  const counts = studies.reduce((acc, s) => { acc[s.status] = (acc[s.status] || 0) + 1; return acc; }, {});
  ...
  return (
    <div style={{ padding: "0 0 40px" }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 24 }}>
        <div>
          <h2 style={{ margin: "0 0 4px", fontSize: 22, fontWeight: 700, color: "#e2e8f0" }}>My Studies</h2>
          <p style={{ margin: 0, fontSize: 13, color: "#64748b" }}>{studies.length} total · persisted across deploys</p>
        </div>
      ...
      {loading ? (
        <div style={{ textAlign: "center", padding: "60px 0", color: "#64748b" }}>Loading studies…</div>
      ) : error ? (
        <div style={{ background: "rgba(239,68,68,0.08)", border: "1px solid rgba(239,68,68,0.2)",
          borderRadius: 10, padding: 20, textAlign: "center", color: "#fca5a5", fontSize: 13 }}>
          {error}<br />
          <button onClick={fetchStudies} style={{ marginTop: 12, padding: "7px 16px", borderRadius: 8,
            border: "none", background: "#ef4444", color: "#fff", cursor: "pointer", fontSize: 12 }}>Retry</button>
        </div>
      ) : filtered.length === 0 ? (
        <div style={{ textAlign: "center", padding: "60px 0", color: "#64748b" }}>
          <div style={{ fontSize: 32, marginBottom: 12 }}>🔬</div>
          {filter === "all" ? "No studies yet — launch one from the Launch tab" : `No ${filter} studies`}
        </div>
      ) : (
        <div style={{ display: "flex", flexDirection: "column", gap: 10, animation: "fade-in 0.4s ease" }}>
          {filtered.map(study => (
            <StudyCard key={study.id || study.study_id} study={study}
              session={session} onStop={handleStop} onDelete={handleDelete} />
          ))}
        </div>
      )}
    </div>
