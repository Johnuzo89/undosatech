...
function Badge({ status }) {
  const m = { pending:{bg:'#f3f4f6',c:'#374151',b:'#e5e7eb'}, running:{bg:'#f5f3ff',c:'#6d28d9',b:'#ddd6fe'}, completed:{bg:'#ecfdf5',c:'#065f46',b:'#a7f3d0'}, failed:{bg:'#fef2f2',c:'#991b1b',b:'#fecaca'} }
  const s = m[status]||m.pending
  return <span style={{background:s.bg,color:s.c,border:`1px solid ${s.b}`,padding:'2px 10px',borderRadius:20,fontSize:12,fontWeight:600,display:'inline-flex',alignItems:'center',gap:5}}>
    {status==='running'&&<span style={{width:6,height:6,borderRadius:'50%',background:'#7c3aed',animation:'pulse 1.2s infinite'}}/>}{status}
  </span>
}
...
function Stat({ label, value, color, sub }) {
  return <div style={{background:'#f9fafb',borderRadius:10,padding:'12px 16px',border:'1px solid #f3f4f6'}}>
    ...
    <div style={{fontSize:22,fontWeight:700,color:color||'#111',lineHeight:1}}>{value}</div>
...
function LaunchForm({ onLaunched }) {
  ...
  const [form, setForm] = useState({
    study_name:'', researcher_name:'', institution:'',
  ...
  const DATASETS = [
    ...
    {v:'upload',     l:'Upload your own data'},
  ]
  const ARCHS = [{v:'resnet18',l:'ResNet-18 (recommended)'},{v:'resnet50',l:'ResNet-50 (deeper)'},{v:'cnn',l:'Lightweight CNN (fastest)'}]
  ...
  const submit = async e => {
    ...
    try {
      ...
      fd.append('nodes', JSON.stringify([
        {node_id:'moorfields-001', institution_name:'NHS Moorfields Eye Hospital', partition_id:0},
        {node_id:'edinburgh-001',  institution_name:'University of Edinburgh Medical School', partition_id:1},
      ]))
  ...
  return (
    <form onSubmit={submit} style={{maxWidth:580}}>
      <h1 style={{fontSize:22,fontWeight:700,marginBottom:6}}>Launch a federated study</h1>
      <p style={{fontSize:13,color:'#6b7280',marginBottom:22,maxWidth:500}}>Real ResNet-18 training across two institution nodes. Data never leaves its node — only weight gradients travel via FedAvg.</p>

      <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12}}>
        <div><label style={S.lbl}>Study name *</label><input style={S.inp} required placeholder="Retinal OCT — UK Cohort" value={form.study_name} onChange={e=>set('study_name',e.target.value)}/></div>
        <div><label style={S.lbl}>Your name *</label><input style={S.inp} required placeholder="Dr. John Ohanebo" value={form.researcher_name} onChange={e=>set('researcher_name',e.target.value)}/></div>
      </div>
      <label style={S.lbl}>Institution *</label>
      <input style={S.inp} required placeholder="University of Dundee" value={form.institution} onChange={e=>set('institution',e.target.value)}/>

      <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12}}>
        <div>
          <label style={S.lbl}>Dataset</label>
          <select style={{...S.inp,marginBottom:0}} value={form.dataset} onChange={e=>set('dataset',e.target.value)}>
            {DATASETS.map(d=><option key={d.v} value={d.v}>{d.l}</option>)}
          </select>
        </div>
        <div>
          <label style={S.lbl}>Architecture</label>
          <select style={{...S.inp,marginBottom:0}} value={form.architecture} onChange={e=>set('architecture',e.target.value)}>
      ...
      {form.dataset === 'upload' && (
        <div style={{margin:'12px 0'}}>
          <label style={S.lbl}>Upload dataset file</label>
          <div onDragOver={e=>{e.preventDefault();setDrag(true)}} onDragLeave={()=>setDrag(false)}
            ...
            <div style={{fontSize:13,fontWeight:500,color:file?'#059669':'#374151'}}>{file?file.name:'Drop NPZ, CSV, or ZIP'}</div>
          </div>
      ...
      <div style={{background:'#f0f9ff',border:'1px solid #bae6fd',borderRadius:8,padding:'10px 14px',marginBottom:14,fontSize:12,color:'#0369a1'}}>
        🔒 <strong>Zero raw data transfer.</strong> Each institution trains on its local partition. Only model weight gradients are aggregated via FedAvg. Full audit trail generated.
      </div>
      ...
      <button type="submit" disabled={busy} style={{width:'100%',padding:12,background:busy?'#93c5fd':'#1d4ed8',color:'#fff',borderRadius:8,fontWeight:700,fontSize:15,cursor:busy?'not-allowed':'pointer',border:'none'}}>
...
// ── Study view — POLLING based (no WebSocket) ─────────────────────────────────
...
function StudyView({ studyId, onBack }) {
  ...
  const addLog = useCallback((msg, type='info') =>
    setLog(l => [...l, {ts: new Date().toLocaleTimeString(), msg, type}].slice(-500)), [])
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      try {
        ...
        // Generate log entries from state changes
        if (prev.status === null) {
          addLog(`✅ Connected — study: ${data.study_name}`, 'success')
          addLog(`   Dataset: ${data.dataset}  ·  Architecture: ${data.architecture}  ·  ${data.num_rounds} rounds`, 'info')
          if (data.data_description) addLog(`   Data: ${data.data_description}`, 'info')
        }

        if (data.status === 'running' && prev.status !== 'running') {
          addLog(`⚡ Training started — FedAvg across ${data.nodes?.length || 2} nodes`, 'info')
        }
        ...
        if (rounds.length > prev.round) {
          for (let i = prev.round; i < rounds.length; i++) {
            const r = rounds[i]
            addLog(`⚡ Round ${r.round}/${data.num_rounds} — distributing global model…`, 'info')
            if (r.node_metrics) {
              r.node_metrics.forEach(n => {
                addLog(`   🏥 ${n.institution}`, 'node')
                addLog(`      acc ${(n.accuracy*100).toFixed(1)}%  ·  loss ${n.loss.toFixed(4)}  ·  lr ${n.learning_rate}  ·  ${n.num_examples} samples`, 'node')
              })
            }
        ...
        if (data.status === 'completed' && prev.status !== 'completed') {
          addLog(`🎉 Training complete! Final accuracy: ${(data.final_accuracy*100).toFixed(1)}%`, 'success')
          addLog(`   Model saved to: ${data.model_path}`, 'info')
          apiFetch(`/studies/${studyId}/audit`).then(a => setAudit(a.events)).catch(()=>{})
        }
  ...
  const chart = (job?.round_results || []).map(r => ({
    round: `R${r.round}`,
    acc:   +(r.global_accuracy*100).toFixed(2),
  ...
  return (
    <div>
      ...
      {job ? <>
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:16,flexWrap:'wrap',gap:10}}>
          <div>
            ...
            <div style={{fontSize:12,color:'#9ca3af',marginBottom:6}}>{job.researcher_name} · {job.institution}</div>
            <div style={{display:'flex',gap:8,alignItems:'center',flexWrap:'wrap'}}>
              <Badge status={job.status}/>
              {job.architecture&&<span style={{fontSize:11,background:'#eff6ff',color:'#1e40af',padding:'2px 8px',borderRadius:20,border:'1px solid #bfdbfe',fontWeight:500}}>{job.architecture}</span>}
              {job.dataset&&<span style={{fontSize:11,background:'#f5f3ff',color:'#5b21b6',padding:'2px 8px',borderRadius:20,border:'1px solid #ddd6fe',fontWeight:500}}>{job.dataset}</span>}
            </div>
          </div>
          {job.final_accuracy!=null&&
            <div style={{textAlign:'right'}}>
              <div style={{fontSize:36,fontWeight:800,color:'#059669',lineHeight:1}}>{(job.final_accuracy*100).toFixed(1)}%</div>
              <div style={{fontSize:11,color:'#9ca3af'}}>final accuracy</div>
            </div>
          }
        </div>
        ...
        {job.status==='running'&&job.num_rounds>0&&(
          <div style={{marginBottom:14}}>
            <div style={{display:'flex',justifyContent:'space-between',fontSize:12,color:'#6b7280',marginBottom:4}}>
              <span>Round {job.current_round} of {job.num_rounds}</span>
              <span>{Math.round(((job.current_round||0)/job.num_rounds)*100)}%</span>
            </div>
            <div style={{height:7,background:'#f3f4f6',borderRadius:4,overflow:'hidden'}}>
              <div style={{height:'100%',width:`${((job.current_round||0)/job.num_rounds)*100}%`,background:'linear-gradient(90deg,#1d4ed8,#7c3aed)',borderRadius:4,transition:'width 0.6s ease'}}/>
            </div>
          </div>
        )}

        {job.data_description&&
          <div style={{background:'#f0fdf4',border:'1px solid #bbf7d0',borderRadius:8,padding:'8px 14px',fontSize:12,color:'#065f46',marginBottom:14}}>
            📊 {job.data_description}
          </div>
        }
        ...
        <div style={{display:'flex',gap:8,marginBottom:14,flexWrap:'wrap'}}>
          {['live','chart','per-class','nodes','interpretability','audit'].map(t=>
        ...
        {tab==='live'&&(
          <div style={S.card}>
            <div style={{fontSize:12,fontWeight:600,marginBottom:10,display:'flex',justifyContent:'space-between'}}>
              ...
              <span style={{fontWeight:400,color:'#9ca3af'}}>{log.length} events · polling every 2s</span>
            </div>
            <div ref={logRef} style={{fontFamily:'monospace',fontSize:12,maxHeight:400,overflowY:'auto',display:'flex',flexDirection:'column',gap:2}}>
              {log.map((l,i)=>(
                <div key={i} style={{display:'flex',gap:10}}>
                  <span style={{color:'#d1d5db',flexShrink:0}}>{l.ts}</span>
                  <span style={{color:logCol[l.type]||'#374151'}}>{l.msg}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        {tab==='chart'&&(
          <div style={S.card}>
            ...
            {chart.length===0
              ? <div style={{color:'#9ca3af',fontSize:13}}>Chart updates as rounds complete…</div>
        ...
        {tab==='per-class'&&(
          <div style={S.card}>
            ...
            {lastRound?.per_class_accuracy
              ? (()=>{
                  const labels = job.interpretability?.class_labels || lastRound.per_class_accuracy.map((_,i)=>`Class ${i}`)
                  const data = lastRound.per_class_accuracy.map((acc,i)=>({name:labels[i],acc}))
                  return (
              ...
              : <div style={{color:'#9ca3af',fontSize:13}}>Available after round 1 completes.</div>
            }
          </div>
        )}
        ...
        {tab==='nodes'&&(
          <div>
            {lastRound?.node_metrics?.map(n=>(
              <div key={n.node_id} style={{...S.card,display:'flex',alignItems:'center',gap:14}}>
                <div style={{width:40,height:40,borderRadius:'50%',background:'#eff6ff',display:'flex',alignItems:'center',justifyContent:'center',fontSize:20,flexShrink:0}}>🏥</div>
                <div style={{flex:1}}>
                  <div style={{fontWeight:600,fontSize:13,marginBottom:2}}>{n.institution}</div>
                  <div style={{fontSize:12,color:'#9ca3af'}}>{n.num_examples} samples · lr {n.learning_rate}</div>
                </div>
                <div style={{textAlign:'right'}}><div style={{fontWeight:700,color:'#1d4ed8',fontSize:16}}>{(n.accuracy*100).toFixed(1)}%</div><div style={{fontSize:11,color:'#9ca3af'}}>local acc</div></div>
                <div style={{textAlign:'right'}}><div style={{fontWeight:600,fontSize:13}}>{n.loss.toFixed(4)}</div><div style={{fontSize:11,color:'#9ca3af'}}>loss</div></div>
                <span style={{background:'#ecfdf5',color:'#065f46',fontSize:11,padding:'2px 9px',borderRadius:20,border:'1px solid #a7f3d0',fontWeight:500}}>{n.governance_status}</span>
              </div>
            ))||<div style={{...S.card,color:'#9ca3af',fontSize:13}}>Node metrics appear after round 1.</div>}
          </div>
        )}
        ...
        {tab==='interpretability'&&(
          <div style={S.card}>
            {job.interpretability?<>
              ...
              {job.interpretability.class_labels?.length>0&&(
                <div style={{marginBottom:14}}>
                  ...
                  <div style={{display:'flex',flexWrap:'wrap',gap:6}}>
                    {job.interpretability.class_labels.map((l,i)=>(
                      <span key={i} style={{background:'#f5f3ff',color:'#5b21b6',fontSize:12,padding:'3px 10px',borderRadius:20,border:'1px solid #ddd6fe',fontWeight:500}}>{l}</span>
                    ))}
                  </div>
                </div>
              )}
              ...
              {job.interpretability.top_features.map((f,i)=>(
                <div key={i} style={{marginBottom:10}}>
                  <div style={{display:'flex',justifyContent:'space-between',marginBottom:3,fontSize:13}}>
                    <span>{f.feature}</span>
                    <span style={{fontWeight:600,color:f.direction==='positive'?'#059669':'#dc2626'}}>{f.direction==='positive'?'+':'−'}{(f.importance*100).toFixed(1)}%</span>
                  </div>
                  <div style={{height:6,background:'#f3f4f6',borderRadius:3}}>
                    <div style={{height:'100%',width:`${f.importance*100}%`,background:f.direction==='positive'?'#1d4ed8':'#dc2626',borderRadius:3}}/>
                  </div>
                </div>
              ))}
        ...
        {tab==='audit'&&(
          <div style={S.card}>
            ...
            <div style={{fontFamily:'monospace',fontSize:11,maxHeight:380,overflowY:'auto'}}>
              {audit.length===0
                ? <span style={{color:'#9ca3af'}}>Available after training completes.</span>
                : audit.slice().reverse().map(e=>(
                  <div key={e.event_id} style={{padding:'5px 0',borderBottom:'1px solid #f3f4f6',display:'flex',gap:10}}>
                    ...
                    {e.event_type==='study_completed'&&<span style={{color:'#059669'}}>final acc {(e.final_accuracy*100).toFixed(1)}%</span>}
                  </div>
                ))
              }
            </div>
      ...
      </> : <div style={{color:'#9ca3af',padding:40,textAlign:'center',fontSize:14}}>Loading study…</div>}
    </div>
  )
}
...
function StudiesList({ studies, onSelect }) {
  if (!studies.length) return (
    <div style={{textAlign:'center',padding:'60px 20px',color:'#9ca3af'}}>
      ...
      <div style={{fontSize:16,fontWeight:600,color:'#374151',marginBottom:6}}>No studies yet</div>
      <div style={{fontSize:13}}>Go to Launch to start your first federated training run.</div>
    </div>
  )
  return (
    <div style={{background:'#fff',border:'1px solid #e5e7eb',borderRadius:12}}>
      {studies.map((s,i)=>(
        <div key={s.study_id} onClick={()=>onSelect(s.study_id)}
          ...
          <div style={{flex:1,minWidth:0}}>
            ...
            <div style={{fontSize:11,color:'#9ca3af'}}>{s.researcher_name} · {s.dataset} · {s.architecture} · {s.num_rounds} rounds</div>
          </div>
...
export default function App() {
  ...
  const nav = (t, label, badge) => (
    <button onClick={()=>{setTab(t);if(t!=='studies')setSelected(null)}}
      style={{padding:'7px 16px',borderRadius:8,fontSize:13,fontWeight:tab===t?600:400,cursor:'pointer',border:'none',
              background:tab===t?'#1d4ed8':'transparent',color:tab===t?'#fff':'#9ca3af',display:'flex',alignItems:'center',gap:6}}>
    ...
    <div style={{minHeight:'100vh',background:'#f9fafb'}}>
      <header style={{background:'#111827',padding:'0 24px',height:52,display:'flex',alignItems:'center',gap:14,position:'sticky',top:0,zIndex:10}}>
        <div style={{color:'#fff',fontWeight:700,fontSize:15,marginRight:8}}>
          UndosaTech <span style={{fontSize:10,fontWeight:400,color:'#6b7280'}}>Federated Research Platform</span>
        </div>
        {nav('launch','🚀 Launch',0)}
      ...
      <div style={{maxWidth:800,width:'100%',margin:'0 auto',padding:'32px 20px'}}>
        ...
        {tab==='studies'&&!selected&&<>
          <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:20}}>
            ...
            <div style={{fontSize:13,color:'#9ca3af'}}>
              {running>0&&<span style={{color:'#7c3aed',fontWeight:600,marginRight:12}}>⚡ {running} running</span>}
      ...
      <style>{`
        ...
        *{box-sizing:border-box}
        body{margin:0;font-family:-apple-system,BlinkMacSystemFont,'Inter',sans-serif;background:#f9fafb;color:#111827;font-size:14px;-webkit-font-smoothing:antialiased}
